...
const contentOptions = [
  { value: "raw", text: "原始数据" },
  { value: "schema", text: "字段结构" },
  { value: "logs", text: "变更记录" },
  { value: "quality", text: "质量报告" },
];
  ...
  return (
    <div className="ex-progress">
      <div className="ex-progress-track">
        <div className="ex-progress-fill" style={{ width: `${p}%`, background: color }} />
      </div>
      <span className="ex-progress-text">{p}%</span>
    </div>
  );
}
    ...
    >
      {phase === "config" && (
        <div className="ex-body">
          {ctx?.scope === "single" ? (
            <DivMessage display type="default" text={`将导出数据集「${ctx.recordName}」`} enableDisposeTimeOut={false} className="ex-alert" />
          ) : (
          ...
          <div className="ex-block ex-row2">
            <div>
              <label>分卷大小</label>
              <div className="ex-num-wrap">
                <Spinner min={1} max={10} doNotFocusWhenValueUpdate value={splitGB} onChange={(v) => setSplitGB(Number(v) || 1)} />
                <span className="ex-num-suffix">GB</span>
              </div>
            </div>
            <div>
              <label>预计体积</label>
      ...
      {phase === "done" && (
        <div className="ex-body">
          <DivMessage display type="success" text="导出完成" title="文件已生成并通过完整性校验，可立即下载。" enableDisposeTimeOut={false} className="ex-alert" />
