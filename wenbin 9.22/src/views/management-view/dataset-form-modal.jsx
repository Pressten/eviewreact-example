...
export default function DatasetFormModal({ open, mode, initial, onCancel, onSubmit }) {
  ...
  const handleSuccess = (values) => {
    ...
    setTimeout(() => {
      setSaving(false);
      const out = { ...values, qualityTarget: Array.isArray(values.qualityTarget) ? values.qualityTarget[0] : values.qualityTarget };
      onSubmit(out);
    }, 500);
  };
  ...
  return (
    <Dialog
      ...
      buttons={[
        ...
        { text: saving ? "保存中..." : (mode === "edit" ? "保存修改" : "立即创建"), status: "primary", disabled: saving, onClick: () => formRef.current?.submit() },
      ]}
    >
      <Form
        ref={formRef}
      ...
      >
        ...
        <Form.Item name="savePolicy" label="同步策略">
          <RadioGroup data={[{ value: "realtime", text: "实时" }, { value: "daily", text: "每日" }, { value: "weekly", text: "每周" }]} />
        </Form.Item>
        <Form.Item name="fields" label="字段数量" rules={[{ required: true }]}>
          <Spinner min={1} max={500} />
        </Form.Item>
        <Form.Item name="access" label="访问权限">
          <CheckboxGroup data={[{ text: "只读", value: "read" }, { text: "可编辑", value: "edit" }, { text: "可导出", value: "export" }, { text: "可分享", value: "share" }]} />
        </Form.Item>
        <Form.Item
          name="isPublic"
          label={
        ...
        <Form.Item name="qualityTarget" label="质量目标分" col={24}>
          <DragInput min={0} max={100} markIndexes={[0, 60, 80, 100]} labelFormat={(v) => ({ formatValue: `${v} 分` })} />
        </Form.Item>
        ...
        >
          <Toggle data={[false, true]} toggled />
        </Form.Item>
      </Form>
    </Dialog>
  );
