...
import AppSegmented from "../../components/app-segmented.jsx";
    ...
    <div className="sy-page">
      ...
      <div className="page-head">
        ...
        <div className="page-head-actions">
          <Button leftIcon={<Icon name="play" size={14} />} text="同步全部" onClick={() => { setTasks((prev) => prev.map((t) => (t.status === "queued" ? { ...t, status: "running" } : t))); message.success("已启动全部排队任务"); }} />
          <Button status="primary" leftIcon={<Icon name="plus" size={14} />} text="新建同步任务" onClick={() => message.info("演示：任务编排器即将上线")} />
        </div>
      </div>
      ...
      <div className="sy-layout">
        <SectionCard icon="refresh-cw" title="任务列表" subtitle="运行中的任务将实时推进进度">
          <div className="sy-toolbar">
            ...
            <Select style={{ width: 150 }} value={src} onChange={(v) => setSrc(v)} options={[{ text: "全部来源", value: "all" }, ...sourceOptions.map((v) => ({ text: v, value: v }))]} />
            <AppSegmented value={seg} onChange={(v) => setSeg(v)} options={[{ label: "全部", value: "all" }, { label: "运行中", value: "running" }, { label: "失败", value: "failed" }]} />
          </div>
              ...
              <div className="sy-task" key={t.id}>
                ...
                <div className="sy-task-foot">
                  ...
                  <div className="sy-task-actions">
                    {t.status === "running" ? (
                      <Button leftIcon={<Icon name="pause" size={13} />} text="暂停" onClick={() => { setStatus(t.id, "queued"); message.info(`「${t.name}」已暂停`); }} />
                    ) : (
                      <Button status="primary" leftIcon={<Icon name="play" size={13} />} text={t.status === "failed" ? "重试" : t.status === "done" ? "重新运行" : "启动"} onClick={() => { setStatus(t.id, "running", t.status === "done"); message.success(`「${t.name}」已启动`); }} />
                    )}
                    <AppDropdown menu={moreMenu(t)}>
