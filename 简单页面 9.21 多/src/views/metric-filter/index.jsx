...
import {
  ...
  IconPlusIcPublicArrowUp,
  IconPlusIcPublicArrowDown,
} from "@nce/icon-plus";
...
function FilterSelect({ label, value, options, onChange }) {
  const mappedOptions = options.map((opt) => ({ text: opt.label, value: opt.value }));
  return (
    <Field label={label}>
      <Select
        enableClear
    ...
    >
      ...
      <div className="metric-filter__actions">
        <Button
          ...
          rightIcon={expanded ? <IconPlusIcPublicArrowUp /> : <IconPlusIcPublicArrowDown />}
          onClick={() => setExpanded((prev) => !prev)}
        />
        <Button text="重置" leftIcon={<IconPlusIcPublicRefresh />} onClick={resetFilters} />
        <Button status="primary" text="查询" leftIcon={<IconPlusIcPublicSearch />} onClick={applyFilters} />
      </div>
    </PanelCard>
  );
}
