...
import { AppAvatar, SimpleProgress, KeyValueList, Dropdown } from "../../shared/common.jsx";
...
export default function ManagementView() {
  ...
  const moveToRecycle = (rows) => {
    ...
    setSelectedKeys([]);
    toast.warning(`已将 ${rows.length} 个数据集移入回收站，30 天内可恢复`);
  };
  ...
  const batchOnline = () => {
    ...
    confirm({
      ...
      onOk: () => {
        ...
        setSelectedKeys([]);
        toast.success(`${targets.length} 个数据集已发布上线`);
      },
    });
  };
      ...
      <div className="mg-qpop">
        {rows.map(([k, v]) => (
          <div className="mg-qpop-row" key={k}>
            <span>{k}</span>
            <SimpleProgress percent={v} size="small" showInfo={false} />
            <i>{v}</i>
          </div>
        ))}
      </div>
  ...
  const columns = [
    { title: "ID", key: "id", display: false },
    {
      ...
      render: (v, row) => (
        <div className="mg-name">
          <button type="button" className="mg-name-link" onClick={() => setDetail(row)} title="点击查看详情">
            {v}
          </button>
          <span className="mg-name-meta">
            {row.id} · {row.fields} 个字段
          </span>
        </div>
      ),
    },
    ...
    { title: "存储量", key: "sizeMB", width: 100, render: (v) => fmtSize(v) },
    {
      ...
      width: 155,
      render: (v) => (
        <TipBox title="质量评分构成" content={qDetail({ quality: v })} direction="top">
    ...
    { title: "更新时间", key: "updated", width: 150, sort: "desc" },
    {
      ...
      render: (_v, row) => (
        <div className="mg-actions">
          <TipBox content="查看详情" type="simple" direction="top">
            <Button size="small" status="text" leftIcon={<Icon name="eye" size={14} />} onClick={() => setDetail(row)}>
              详情
            </Button>
          </TipBox>
          <TipBox content="编辑" type="simple" direction="top">
            <Button size="small" status="text" leftIcon={<Icon name="pencil" size={14} />} onClick={() => setEditRecord(row)}>
              编辑
            </Button>
          </TipBox>
          <Dropdown
            trigger={<Button size="small" status="text" leftIcon={<Icon name="ellipsis" size={14} />} />}
            items={moreMenuItems(row)}
            onSelect={(key) => onMoreSelect(row, key)}
    ...
    <div className="mg-page">
        ...
        <div className="mg-filter-row">
          ...
          <div className="mg-f-item">
            ...
            <DatePicker
              range={[]}
              onOkClick={(obj) => {
                setTimeout(() => set({ range: obj.fromSelectDate ? [obj.fromSelectDate, obj.toSelectDate] : null }), 100);
              }}
              onCancelClick={() => set({ range: null })}
            />
          </div>
          <div className="mg-f-item">
            <label>状态</label>
          ...
          <div className="mg-f-item">
            <label>仅看我维护的</label>
            <Toggle toggled={draft.onlyMine} onToggle={(v) => set({ onlyMine: v })} data={[false, true]} />
          </div>
          ...
          <div className="mg-advanced-inner">
            <div className="mg-f-item">
              ...
              <DragInput
                ...
                markIndexes={[0, 2.5, 5]}
                onChange={(arr) => set({ minQuality: arr[0] })}
              />
            </div>
            <div className="mg-f-item">
              <label>最小记录数</label>
              <div className="mg-input-suffix">
                <Spinner min={0} step={10000} value={draft.minRecords} onChange={(v) => set({ minRecords: v || 0 })} style={{ width: 130 }} />
                <span>条</span>
              </div>
            </div>
            <div className="mg-f-item">
              ...
              <div className="mg-chips">
                {hotTags.map((t) => (
                  <button
                    key={t}
                    type="button"
                    className={`app-tag-chip${chips.includes(t) ? " active" : ""}`}
      ...
      >
        <Table
          ...
          onDoubleClick={(evtRow) => setDetail(evtRow)}
          enableRowExpand
  ...
  function handleCreate(values) {
    ...
    const record = {
      ...
      records: 0,
      sizeMB: 0,
      quality: (values.qualityTarget && values.qualityTarget[0] ? values.qualityTarget[0] : values.qualityTarget || 80) / 20,
      status: "draft",
      updated: now(),
    ...
    setCreateOpen(false);
    toast.success(`数据集「${record.name}」创建成功，已加入草稿列表`);
  }
  ...
  function handleEdit(values) {
    ...
    setEditRecord(null);
    toast.success(`数据集「${values.name}」已更新`);
  }
}
