...
// TODO(eview-react): 顶部水平导航 Menu（antd Menu mode="horizontal"）未覆盖，此处手写补位。
// 真实组件建议待 Menu / HorizontalNav 规格就绪后替换；当前仅做选中态展示，不接入路由。
export default function HeaderBar() {
  ...
  return (
    <header className="header-bar">
      ...
      <nav className="header-bar__nav" aria-label="主导航">
        {topNav.map((item) => (
          <button
            key={item.key}
            type="button"
      ...
      <div className="header-bar__tools">
        ...
        <Badge content={6}>
          <IconButton iconName={<Icon name="bell" size={16} />} tipText={t('top.notify')} onClick={() => {}} />
        </Badge>

        <IconButton iconName={<Icon name="circle-question-mark" size={16} />} tipText={t('top.help')} onClick={() => {}} />
        ...
        <IconButton iconName={<Icon name="log-out" size={16} />} tipText={t('top.logout')} onClick={() => {}} />
      </div>
    </header>
  );
}
