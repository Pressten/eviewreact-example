      ...
      {record ? (
        <div className="dw-body">
          ...
          <div className="dw-desc">
            <h3 className="dw-desc-title">基本信息</h3>
            <KeyValueList
              columns={2}
              items={[
                { label: "数据集 ID", value: <code className="dw-code">{record.id}</code>, full: true },
                ...
                { label: "访问权限", value: <>{(record.access || ["read"]).map((a) => (<Tag key={a}>{accessLabels[a] || a}</Tag>))}</>, full: true },
                { label: "数据描述", value: record.desc, full: true },
              ]}
            />
          </div>
