// page-header.jsx — Layer 4: 页面头部（面包屑 + 标题 + 全局操作 + 指标概览卡）
// 面包屑/Layout 无对应组件，按 handwrite-templates 手写 nav + AppIcon
    ...
    <header className="page-header">
      <div className="page-header__top">
        ...
        <div className="page-header__actions">
          <IconButton
            iconName={<AppIcon name={isDark ? 'sun' : 'moon'} size={16} />}
            tipText={isDark ? '切换为浅色模式' : '切换为深色模式'}
            onClick={toggleDark}
          />
          <Button leftIcon={<AppIcon name="refresh-cw" size={14} />} text="刷新" />
          <Button leftIcon={<AppIcon name="download" size={14} />} text="导出报告" />
          <Button status="primary" leftIcon={<AppIcon name="plus" size={14} />} text="新建指标" />
        </div>
      </div>
