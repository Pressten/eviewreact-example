...
import { useNotice, NoticeBar, useConfirm, ConfirmDialog, AppAvatar } from "../../shared/ui-helpers.jsx";
import "./index.css";
...
export default function RecycleView() {
  ...
  const columns = [
    ...
    { title: "Reason", key: "reason", display: false },
    {
      title: "数据集",
      key: "name",
    ...
    },
    {
      ...
      render: (_v, _rowData, _opts, row) => {
        const r = filtered.find((d) => d.id === row.id);
        if (!r) return null;
        return (
          <div className="mg-actions">
            <Button status="text" text="恢复" leftIcon={<Icon name="rotate-ccw" size={13} />} onClick={() => restore(r)} />
            <Button status="risk" text="彻底删除" leftIcon={<Icon name="x" size={13} />} onClick={() => destroy(r)} />
          </div>
        );
      },
    },
    ...
    <div className="rc-page">
      ...
      <div className="page-head">
        ...
        <div className="page-head-actions">
          <Button status="risk" text="清空回收站" disabled={recycleItems.length === 0} leftIcon={<Icon name="trash-2" size={14} />} onClick={clearAll} />
        </div>
      </div>
      ...
      <SectionCard icon="trash-2" title="已删除数据" subtitle={`共 ${filtered.length} 条结果`}>
        <div className="rc-toolbar">
          <TextField style={{ width: 260 }} value={kw} onChange={(v) => setKw(v)} placeholder="搜索数据集名称 / ID" />
          <Select style={{ width: 150 }} value={type} onChange={(v) => setType(v)} options={[{ value: "all", text: "全部类型" }, ...typeOptions.map((v) => ({ value: v, text: v }))]} />
        </div>
        <Table
          columns={columns}
          dataset={tableData}
