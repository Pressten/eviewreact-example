...
import { useNotice, NoticeBar, useConfirm, ConfirmDialog, AppAvatar, AppDropdown, KeyValueList, SimpleProgress, CheckableTag } from "../../shared/ui-helpers.jsx";
import "./index.css";
...
export default function ManagementView() {
  ...
  const columns = [
    { title: "ID", key: "id", display: false },
    {
      ...
      key: "name",
      width: 250,
      render: (v, _rowData, _opts, row) => {
        const r = filtered.find((d) => d.id === row.id) || row;
        return (
    ...
    { title: "存储量", key: "sizeMB", width: 100, render: (v) => fmtSize(v) },
    {
      ...
      width: 155,
      render: (v) => (
        <TipBox type="simple" content={qDetail({ quality: v })} direction="top">
    ...
    { title: "更新时间", key: "updated", width: 150, sort: "desc" },
    {
      ...
      key: "actions",
      width: 175,
      render: (_v, _rowData, _opts, row) => {
        const r = filtered.find((d) => d.id === row.id) || row;
        return (
          <div className="mg-actions">
            <TipBox type="simple" content="查看详情" direction="top">
              <Button status="text" text="详情" leftIcon={<Icon name="eye" size={14} />} onClick={() => setDetail(r)} />
            </TipBox>
            <TipBox type="simple" content="编辑" direction="top">
              <Button status="text" text="编辑" leftIcon={<Icon name="pencil" size={14} />} onClick={() => setEditRecord(r)} />
            </TipBox>
            <AppDropdown items={moreMenuItems(r)} onSelect={(key) => onMoreSelect(key, r)}>
    ...
    <div className="mg-page">
      ...
      <div className="page-head">
        ...
        <div className="page-head-actions">
          <Button text="刷新" leftIcon={<Icon name="rotate-cw" size={14} />} onClick={refresh} />
          <Button status="primary" text="新建数据集" leftIcon={<Icon name="plus" size={14} />} onClick={() => setCreateOpen(true)} />
        </div>
      </div>
      ...
      >
        <div className="mg-filter-row">
            ...
            <DatePicker
              ...
              onOkClick={(obj) => {
                if (obj && obj.fromSelectDate && obj.toSelectDate) {
                  setTimeout(() => set({ range: [obj.fromSelectDate, obj.toSelectDate] }), 0);
                }
              }}
            />
          </div>
          ...
          <div className="mg-f-item">
            <label>仅看我维护的</label>
            <Switch toggled={draft.onlyMine} onToggle={(v) => set({ onlyMine: v })} />
          </div>
          <div className="mg-f-actions">
            ...
            />
            <Button status="primary" text="查询" leftIcon={<Icon name="search" size={14} />} onClick={() => setApplied({ ...draft })} />
          </div>
        </div>
          ...
          <div className="mg-advanced-inner">
            <div className="mg-f-item">
              ...
              <DragInput
                ...
                markIndexes={[0, 2.5, 5]}
                onChange={(v) => set({ minQuality: Array.isArray(v) ? v[0] : v })}
              />
            </div>
            <div className="mg-f-item">
              <label>最小记录数</label>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                <Spinner min={0} step={10000} value={draft.minRecords} onChange={(v) => set({ minRecords: v || 0 })} style={{ width: 160 }} />
                <span>条</span>
              </span>
            </div>
            <div className="mg-f-item">
            ...
            <div className="mg-f-item">
              ...
              <div className="mg-chips">
                {hotTags.map((t) => (
                  <CheckableTag key={t} checked={chips.includes(t)} onChange={(c) => setChips((prev) => (c ? [...prev, t] : prev.filter((x) => x !== t)))}>
                    {t}
                  </CheckableTag>
                ))}
              </div>
      ...
      {selectedKeys.length > 0 && (
        <div className="mg-batch-bar">
          ...
          <Button text="批量导出" leftIcon={<Icon name="download" size={13} />} onClick={() => setExportCtx({ scope: "selection", count: selectedKeys.length, filteredCount: filtered.length })} />
          <Button text="批量上线" leftIcon={<Icon name="check" size={13} />} onClick={batchOnline} />
          <Button status="risk" text="移入回收站" leftIcon={<Icon name="trash-2" size={13} />} onClick={batchRecycle} />
      ...
      <SectionCard
        ...
        extra={
          <Button text="导出当前结果" leftIcon={<Icon name="download" size={13} />} onClick={() => setExportCtx({ scope: "filtered", count: filtered.length, filteredCount: filtered.length })} />
        }
      >
        <Table
          columns={columns}
