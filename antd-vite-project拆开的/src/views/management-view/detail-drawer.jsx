...
import { useNotice, NoticeBar, KeyValueList, SimpleProgress } from "../../shared/ui-helpers.jsx";
...
const tabKeys = ["fields", "logs", "tasks"];
      ...
      {record ? (
        <div className="dw-body">
          ...
          <div className="dw-extra-bar">
            ...
            />
            <Button text="导出" leftIcon={<Icon name="download" size={13} />} onClick={() => onExport(record)} />
          </div>
          ...
          <Tab
            ...
            draggable={false}
            onClick={(index) => setTab(tabKeys[index] || "fields")}
            className="dw-tabs"
          >
            <TabItem
              title={
          ...
          <div className="dw-footer">
            <Button status="risk" text="下线数据集" leftIcon={<Icon name="x" size={14} />} onClick={() => onOffline(record)} />
            <div className="dw-footer-right">
              <Button status="primary" text="编辑数据集" leftIcon={<Icon name="pencil" size={14} />} onClick={() => onEdit(record)} />
            </div>
          </div>
        </div>
      ) : null}
