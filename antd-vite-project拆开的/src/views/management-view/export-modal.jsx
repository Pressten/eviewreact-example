...
import { useNotice, NoticeBar, SimpleProgress } from "../../shared/ui-helpers.jsx";
    ...
    >
      ...
      {phase === "config" && (
        <div className="ex-body">
          {ctx?.scope === "single" ? (
            <DivMessage display type="default" text={`将导出数据集「${ctx.recordName}」`} enableDisposeTimeOut={false} className="ex-alert" style={{ marginBottom: 12 }} />
          ) : (
          ...
          <div className="ex-block">
            <label>包含内容</label>
            <CheckboxGroup
              data={contentOptions.map((o) => ({ text: o.label, value: o.value }))}
              value={content}
              onChange={(...args) => setContent(args[0])}
            />
          </div>
          ...
          <div className="ex-footer">
            ...
            <Button status="primary" text="开始导出" leftIcon={<Icon name="download" size={14} />} onClick={start} />
          </div>
        </div>
      )}
      {phase === "running" && (
        <div className="ex-body ex-running">
          ...
          <SimpleProgress percent={percent} status="normal" />
      ...
      {phase === "done" && (
        <div className="ex-body">
          <DivMessage display type="success" text="导出完成" enableDisposeTimeOut={false} className="ex-alert" style={{ marginBottom: 12 }}>
