// =============================================================================
// 冻结数据契约（Phase 0 骨架 agent 产出，对页面 agent 只读）
// -----------------------------------------------------------------------------
// 所有视图 mock 数据与下拉 options 集中于此；页面 agent 一律 `import { ... } from '../data.js'`
// （相对路径按所在目录自调：src/views/<name>/ → ../../data.js；src/components/<name>/ → ../../data.js）。
// 禁止页面 agent 改写本文件；如需新增字段，提请编排器回到骨架 agent 处理。
// 数据语义字段对齐原始 index.page.html，未做任何裁剪。
// =============================================================================

// ── 1. 指标清单（metric-list 视图数据源） ──────────────────────────────────
// 字段：id / code / name / domain / source / frequency / owner / score / rows / status / updatedAt
...
// ── 2. 采集明细（record-detail 视图数据源） ──────────────────────────────────
// 字段：id / batch / task / node / partition / rows / cost / quality / status / finishedAt
...
// ── 3. 血缘溯源（lineage-trace 视图数据源） ──────────────────────────────────
// 字段：id / nodeCode / level / nodeType / nodeName / tableName / owner / rows / latency / status / updatedAt
