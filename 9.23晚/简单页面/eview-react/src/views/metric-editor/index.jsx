...
function MetricEditor() {
  ...
  useEffect(() => {
    ...
    if (isEdit && editor.record) {
      ...
      formRef.current?.setFieldsValue({ ...CREATE_DEFAULTS });
    }
  ...
  return (
    ...
    >
      <Form
        ...
        onFailed={() => {}}
      >
        <Form.Item col={24} colon={false} className="metric-editor__section">
          <h3 className="metric-editor__section-title">基本信息</h3>
        </Form.Item>
        <Form.Item label="指标名称" name="name" rules={[{ required: true }]} col={24}>
          <TextField placeholder="例如：集群 CPU 平均使用率" />
        </Form.Item>
        <Form.Item label="指标编码" name="code" rules={[{ required: true }]} col={12}>
          <TextField placeholder="例如：res.cpu.usage.avg" validator={validateCode} />
        </Form.Item>
        <Form.Item label="指标分类" name="category" rules={[{ required: true }]} col={12}>
          <Select options={CATEGORY_OPTIONS} defaultLabel="请选择" />
        </Form.Item>
        <Form.Item label="统计周期" name="cycle" rules={[{ required: true }]} col={12}>
        ...
        <Form.Item label="统计维度" name="dimension" rules={[{ required: true }]} col={12}>
          <Select options={DIMENSION_OPTIONS} defaultLabel="请选择" />
        </Form.Item>
        <Form.Item label="统计单位" name="unit" rules={[{ required: true }]} col={12}>
          <Select options={UNIT_OPTIONS} defaultLabel="请选择" />
        </Form.Item>
        <Form.Item label="目标值" name="target" rules={[{ required: true }]} col={12}>
          <Spinner min={0} precision={2} doNotFocusWhenValueUpdate />
        </Form.Item>
        <Form.Item label="指标极性" name="polarity" rules={[{ required: true }]} col={12}>
          <Select options={POLARITY_OPTIONS} defaultLabel="请选择" />
        </Form.Item>
        <Form.Item label="数据来源" name="source" rules={[{ required: true }]} col={12}>
          <Select options={SOURCE_OPTIONS} defaultLabel="请选择" />
        </Form.Item>
        <Form.Item label="责任人" name="owner" rules={[{ required: true }]} col={12}>
        ...
        <Form.Item label="生效日期" name="effectiveAt" rules={[{ required: true }]} col={12}>
          <DatePicker type="date" format="yyyy-MM-dd" required />
        </Form.Item>
        <Form.Item label="指标状态" name="status" rules={[{ required: true }]} col={12}>
