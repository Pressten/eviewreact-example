    ...
    <div className="sy-page">
      ...
      <div className="page-head">
        ...
        <div className="page-head-actions">
          <Button leftIcon={<Icon name="play" size={14} />} onClick={() => { setTasks((prev) => prev.map((t) => (t.status === "queued" ? { ...t, status: "running" } : t))); message.success("已启动全部排队任务"); }}>
            同步全部
          </Button>
          <Button status="primary" leftIcon={<Icon name="plus" size={14} />} onClick={() => message.info("演示：任务编排器即将上线")}>
            新建同步任务
          </Button>
        </div>
      </div>
              ...
              <div className="sy-task" key={t.id}>
                ...
                <div className="sy-task-foot">
                  ...
                  <div className="sy-task-actions">
                    {t.status === "running" ? (
                      <Button size="small" leftIcon={<Icon name="pause" size={13} />} onClick={() => { setStatus(t.id, "queued"); message.info(`「${t.name}」已暂停`); }}>
                        暂停
                      </Button>
                    ) : (
                      <Button size="small" status="primary" leftIcon={<Icon name="play" size={13} />} onClick={() => { setStatus(t.id, "running", t.status === "done"); message.success(`「${t.name}」已启动`); }}>
