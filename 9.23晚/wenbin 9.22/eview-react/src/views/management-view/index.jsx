...
export default function ManagementView() {
  ...
  const full = (row) => filtered.find((d) => d.id === row.id) || row;
      ...
      render: (v, _r, _o, row) => {
        ...
        return (
          <div className="mg-owner">
            <AppAvatar size={26} style={{ background: "var(--primary-container)", color: "var(--primary-fixed)", fontSize: 12 }}>
              {v.slice(0, 1)}
            </AppAvatar>
      ...
      render: (_v, _r, _o, row) => {
        ...
        return (
          <div className="mg-actions">
            <TipBox content="查看详情" trigger="hover" direction="top">
              <Button size="small" status="text" leftIcon={<Icon name="eye" size={14} />} onClick={() => setDetail(r)}>
                详情
              </Button>
            </TipBox>
            <TipBox content="编辑" trigger="hover" direction="top">
            ...
            <Dropdown menu={moreMenu(r)} trigger={["click"]} placement="bottomRight">
              <Button size="small" status="text" leftIcon={<Icon name="ellipsis" size={14} />} />
            </Dropdown>
          </div>
        );
      },
    ...
    <div className="mg-page">
      ...
      >
        <div className="mg-filter-row">
          ...
          <div className="mg-f-item">
            <label>更新时间</label>
            <div className="mg-daterange">
              <input
                type="date"
                className="mg-native-input"
                value={draft.range ? draft.range[0] : ""}
                onChange={(e) => set({ range: [e.target.value, draft.range ? draft.range[1] : ""] })}
              />
              <span>~</span>
              <input
                type="date"
                className="mg-native-input"
                value={draft.range ? draft.range[1] : ""}
                onChange={(e) => set({ range: [draft.range ? draft.range[0] : "", e.target.value] })}
              />
            </div>
          </div>
          <div className="mg-f-item">
          ...
          <div className="mg-f-item">
            <label>仅看我维护的</label>
            <Switch data={[false, true]} toggled={draft.onlyMine} onToggle={(v) => set({ onlyMine: v })} />
          </div>
        ...
        <div className={`mg-advanced${advOpen ? " open" : ""}`}>
          <div className="mg-advanced-inner">
            <div className="mg-f-item">
              ...
              <div className="mg-slider" style={{ maxWidth: 260 }}>
                <input
                  type="range"
                  min={0}
                  max={5}
              ...
              <div className="mg-chips">
                {hotTags.map((t) => (
                  <button
                    key={t}
                    type="button"
                    className={`mg-chip${chips.includes(t) ? " active" : ""}`}
