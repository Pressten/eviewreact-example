...
import { Alert, Button, Checkbox, InputNumber, Modal, Progress, Radio, message } from "antd";
    ...
    >
      {phase === "config" && (
        <div className="ex-body">
          {ctx?.scope === "single" ? (
            <Alert type="info" showIcon message={`将导出数据集「${ctx.recordName}」`} className="ex-alert" />
          ) : (
            <div className="ex-block">
              <label>导出范围</label>
              <Radio.Group value={scope} onChange={(e) => setScope(e.target.value)}>
                <Radio value="selection">仅选中项（{count} 条）</Radio>
                <Radio value="filtered">当前筛选结果（{ctx?.filteredCount ?? count} 条）</Radio>
              </Radio.Group>
            </div>
          )}
          <div className="ex-block">
            <label>导出格式</label>
            <Radio.Group value={format} onChange={(e) => setFormat(e.target.value)} className="ex-fmt">
              {formats.map((f) => (
                <Radio key={f.v} value={f.v} className={`ex-fmt-item${format === f.v ? " active" : ""}`}>
          ...
          <div className="ex-block">
            <label>包含内容</label>
            <Checkbox.Group options={contentOptions} value={content} onChange={setContent} />
          </div>
          <div className="ex-block ex-row2">
            <div>
              <label>分卷大小</label>
              <InputNumber min={1} max={10} value={splitGB} onChange={(v) => setSplitGB(v || 1)} addonAfter="GB" />
            </div>
            <div>
              <label>预计体积</label>
          ...
          <div className="ex-footer">
            ...
            <Button type="primary" icon={<Icon name="download" size={14} />} onClick={start}>
              开始导出
            </Button>
          </div>
        </div>
      ...
      {phase === "done" && (
        <div className="ex-body">
          <Alert type="success" showIcon message="导出完成" description="文件已生成并通过完整性校验，可立即下载。" className="ex-alert" />
