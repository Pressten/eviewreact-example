...
export default function StrategyTable({ onEdit }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "name",
      width: 208,
      render: (value, _rowData, _opts, row) => (
        <Button
          status="text"
          className="strategy-table__link"
          text={displayName(row, lang)}
          onClick={() => onEdit(row)}
        />
      ),
    },
    ...
    },
    {
      ...
      key: "owner",
      width: 104,
      render: (value, _rowData, _opts, row) => displayOwner(row, lang),
    },
    {
  ...
  return (
    <section className="panel-card strategy-table">
      ...
      <Table
        ...
        enableRowExpand
        onRowExpend={(row) => {
          const full = strategyRows.find((d) => d.id === row.id) || row;
          return (
